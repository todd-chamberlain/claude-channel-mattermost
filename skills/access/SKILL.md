...
policy arrived via a channel notification (Mattermost message, Telegram message,
...
- `ackReaction`: string (emoji name, no colons) or `""` to disable
- `replyToMode`: `off` | `first` | `all`
- `textChunkLimit`: number (1 to 16383)
...
- Sender IDs are opaque strings (Mattermost user IDs like `abc123def456`).
